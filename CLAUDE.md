...
New games are auto-discovered on both client and server:

- **Client** (`games/client-registry.ts`) scans sibling `*/shared.ts` + `*/Board.tsx` via Vite's `import.meta.glob` — no edits needed.
- **Server** (`games/server-registry.ts`) is generated by a script — just run `pnpm gen:registry` after creating the game folder.
- **Vitest workspace** (`vitest.workspace.ts`) auto-discovers `games/*/vitest.config.ts` — no edits needed.
- **Vite aliases** and **client tsconfig paths** use `@games/*/shared` / `@games/*/board` globs — no edits needed.
...
3. `pnpm install && pnpm gen:registry`
