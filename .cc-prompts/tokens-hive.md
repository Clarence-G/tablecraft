# Worker W-hive-tokens — hex → token migration

You are migrating hardcoded hex colors in `games/hive/Board.tsx` to
semantic design tokens. Follow the established pattern from the pilot
(liar-bar, already merged) verbatim.

**Hex count in this file (approximate, pre-migration)**: 14
...
Piece fills are game identities: white-player pieces vs black-player pieces. The per-bug accent stroke (queen amber, ant gold, etc.) is decorative. Extract PIECE_COLORS to shared.ts.

## Scope (HARD fence)

**Edit ONLY**:
- `games/hive/Board.tsx`
- `games/hive/shared.ts` IF needed for a game-mechanic palette extraction
- Nothing else.
...
1. **Zero semantic hex remaining**: `rg -o '#[0-9a-fA-F]{6}' games/hive/Board.tsx`
...
3. **Game tests**: `zsh -c 'source ~/.zshenv && cd /Users/bytedance/Projects/tablecraft/games/hive && pnpm exec vitest run 2>&1' | tail -10` — all pass.

## Done criteria

Create `docs/ISSUE_hive-token-migration.md` with:
