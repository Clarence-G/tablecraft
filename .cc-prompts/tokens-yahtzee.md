# Worker W-yahtzee-tokens — hex → token migration

You are migrating hardcoded hex colors in `games/yahtzee/Board.tsx` to
semantic design tokens. Follow the established pattern from the pilot
(liar-bar, already merged) verbatim.

**Hex count in this file (approximate, pre-migration)**: 32
...
#0c1a2e and #2a1f14 are scene-specific dark shadows — may be intentional deeper offsets for the darker yahtzee scene. Confirm by checking --scene-ambience settings before migrating; if confirmed scene-specific, leave as-is or migrate to a --scene-shadow fallback. All other hex migrate normally.

## Scope (HARD fence)

**Edit ONLY**:
- `games/yahtzee/Board.tsx`
- `games/yahtzee/shared.ts` IF needed for a game-mechanic palette extraction
- Nothing else.
...
1. **Zero semantic hex remaining**: `rg -o '#[0-9a-fA-F]{6}' games/yahtzee/Board.tsx`
...
3. **Game tests**: `zsh -c 'source ~/.zshenv && cd /Users/bytedance/Projects/tablecraft/games/yahtzee && pnpm exec vitest run 2>&1' | tail -10` — all pass.

## Done criteria

Create `docs/ISSUE_yahtzee-token-migration.md` with:
