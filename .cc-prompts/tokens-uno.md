# Worker W-uno-tokens — hex → token migration

You are migrating hardcoded hex colors in `games/uno/Board.tsx` to
semantic design tokens. Follow the established pattern from the pilot
(liar-bar, already merged) verbatim.

**Hex count in this file (approximate, pre-migration)**: 31
...
COLOR_PALETTES at line 13-19 IS the game — extract to UNO_COLORS in shared.ts as canonical game-mechanic palette. Shadow-offset hex migrate to --shadow token.

## Scope (HARD fence)

**Edit ONLY**:
- `games/uno/Board.tsx`
- `games/uno/shared.ts` IF needed for a game-mechanic palette extraction
- Nothing else.
...
1. **Zero semantic hex remaining**: `rg -o '#[0-9a-fA-F]{6}' games/uno/Board.tsx`
...
3. **Game tests**: `zsh -c 'source ~/.zshenv && cd /Users/bytedance/Projects/tablecraft/games/uno && pnpm exec vitest run 2>&1' | tail -10` — all pass.

## Done criteria

Create `docs/ISSUE_uno-token-migration.md` with:
