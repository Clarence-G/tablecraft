# Worker W-texas-holdem-tokens — hex → token migration

You are migrating hardcoded hex colors in `games/texas-holdem/Board.tsx` to
semantic design tokens. Follow the established pattern from the pilot
(liar-bar, already merged) verbatim.

**Hex count in this file (approximate, pre-migration)**: 29
...
Felt/placeholder colors are --scene-* based — leave alone. All remaining hex are semantic (destructive for eliminated, warning for all_in, success for showdown-win).

## Scope (HARD fence)

**Edit ONLY**:
- `games/texas-holdem/Board.tsx`
- `games/texas-holdem/shared.ts` IF needed for a game-mechanic palette extraction
- Nothing else.
...
1. **Zero semantic hex remaining**: `rg -o '#[0-9a-fA-F]{6}' games/texas-holdem/Board.tsx`
...
3. **Game tests**: `zsh -c 'source ~/.zshenv && cd /Users/bytedance/Projects/tablecraft/games/texas-holdem && pnpm exec vitest run 2>&1' | tail -10` — all pass.

## Done criteria

Create `docs/ISSUE_texas-holdem-token-migration.md` with:
