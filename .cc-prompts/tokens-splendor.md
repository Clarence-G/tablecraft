# Worker W-splendor-tokens — hex → token migration

You are migrating hardcoded hex colors in `games/splendor/Board.tsx` to
semantic design tokens. Follow the established pattern from the pilot
(liar-bar, already merged) verbatim.

**Hex count in this file (approximate, pre-migration)**: 37
...
GEM_BG_COLORS / GEM_FG_COLORS already structured at lines 28-42 — just add a comment that they're canonical game-mechanic palette. Semantic hex elsewhere (shadows, primary surfaces) migrate to tokens.

## Scope (HARD fence)

**Edit ONLY**:
- `games/splendor/Board.tsx`
- `games/splendor/shared.ts` IF needed for a game-mechanic palette extraction
- Nothing else.
...
1. **Zero semantic hex remaining**: `rg -o '#[0-9a-fA-F]{6}' games/splendor/Board.tsx`
...
3. **Game tests**: `zsh -c 'source ~/.zshenv && cd /Users/bytedance/Projects/tablecraft/games/splendor && pnpm exec vitest run 2>&1' | tail -10` — all pass.

## Done criteria

Create `docs/ISSUE_splendor-token-migration.md` with:
