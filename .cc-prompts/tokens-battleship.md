# Worker W-battleship-tokens — hex → token migration

You are migrating hardcoded hex colors in `games/battleship/Board.tsx` to
semantic design tokens. Follow the established pattern from the pilot
(liar-bar, already merged) verbatim.

**Hex count in this file (approximate, pre-migration)**: 22
...
Ship hull cells use hard blue for ships, red for hits, blue-grey for water. Extract to SHIP_COLORS in shared.ts with fields {hull, hit, miss}.

## Scope (HARD fence)

**Edit ONLY**:
- `games/battleship/Board.tsx`
- `games/battleship/shared.ts` IF needed for a game-mechanic palette extraction
- Nothing else.
...
1. **Zero semantic hex remaining**: `rg -o '#[0-9a-fA-F]{6}' games/battleship/Board.tsx`
...
3. **Game tests**: `zsh -c 'source ~/.zshenv && cd /Users/bytedance/Projects/tablecraft/games/battleship && pnpm exec vitest run 2>&1' | tail -10` — all pass.

## Done criteria

Create `docs/ISSUE_battleship-token-migration.md` with:
