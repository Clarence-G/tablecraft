# Worker W-gomoku-tokens — hex → token migration

You are migrating hardcoded hex colors in `games/gomoku/Board.tsx` to
semantic design tokens. Follow the established pattern from the pilot
(liar-bar, already merged) verbatim.

**Hex count in this file (approximate, pre-migration)**: 6
...
Black/white stone colors are game-mechanic (#1a1108 near-black, #ffffff white). Keep as hex inside a STONE_COLORS const in games/gomoku/shared.ts if not already structured.

## Scope (HARD fence)

**Edit ONLY**:
- `games/gomoku/Board.tsx`
- `games/gomoku/shared.ts` IF needed for a game-mechanic palette extraction
- Nothing else.
...
1. **Zero semantic hex remaining**: `rg -o '#[0-9a-fA-F]{6}' games/gomoku/Board.tsx`
...
3. **Game tests**: `zsh -c 'source ~/.zshenv && cd /Users/bytedance/Projects/tablecraft/games/gomoku && pnpm exec vitest run 2>&1' | tail -10` — all pass.

## Done criteria

Create `docs/ISSUE_gomoku-token-migration.md` with:
