...
export function registerReportsRoutes(router: Router): void {
  ...
  router.post('/reports', async (req: Request, res: Response) => {
    ...
    if (!parsed.success) {
      return void res
        .status(400)
        .json({ ok: false, error: { code: 'BAD_REQUEST', detail: parsed.error.flatten() } });
    }
    ...
    const recent = await db
      ...
      .from(reports)
      .where(
        and(eq(reports.reporterId, userId), sql`${reports.createdAt} > now() - interval '1 hour'`),
      );
  ...
  router.delete('/reports/blocks/:targetUserId', async (req: Request, res: Response) => {
    ...
    await db
      .delete(userBlocks)
      .where(
        and(eq(userBlocks.blockerId, userId), eq(userBlocks.blockedId, req.params.targetUserId)),
      );
    res.json({ ok: true });
  });
  ...
  router.get('/reports/blocks', async (req: Request, res: Response) => {
    ...
    res.json({
      ok: true,
      data: rows.map((r) => ({ userId: r.blockedId, createdAt: r.createdAt })),
    });
  });
}
