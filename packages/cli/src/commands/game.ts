...
export async function gameStateCommand(client: ApiClient, args: string[]): Promise<unknown> {
  ...
  if (!roomId) {
    return {
      ok: false,
      error: 'MISSING_ARGS',
      message: 'Usage: tablecraft game state <roomId>',
      hint: '',
    };
  }
...
export async function gameActionCommand(client: ApiClient, args: string[]): Promise<unknown> {
  ...
  if (!roomId || !actionJson) {
    return {
      ok: false,
      error: 'MISSING_ARGS',
      message: "Usage: tablecraft game action <roomId> '<json>'",
      hint: '',
    };
  }
  ...
  try {
    ...
    return {
      ...
      hint: 'Ensure the action is valid JSON',
    };
  }
...
export async function gameWaitCommand(client: ApiClient, args: string[]): Promise<unknown> {
  ...
  if (!roomId) {
    return {
      ok: false,
      error: 'MISSING_ARGS',
      message: 'Usage: tablecraft game wait <roomId>',
      hint: '',
    };
  }
