...
export function Board({
  ...
  const showClaimPanel = !gameOver && (state.phase === 'playing' || state.phase === 'challenging');

  return (
    <div className="flex-1 text-foreground flex flex-col sm:flex-row p-3 sm:p-4 max-w-3xl lg:max-w-5xl mx-auto w-full gap-3">
      {/* Main column */}
      <div className="flex-1 min-w-0 flex flex-col gap-3 order-2 sm:order-1">
            ...
            return (
              <div key={p.id} className="flex flex-col items-center gap-1.5">
                ...
                <div className="flex items-center gap-1 text-xs text-muted-foreground">
                  {!info?.alive && (
                    <span className="text-destructive font-semibold">{t('out')}</span>
                  )}
      ...
      {/* Claim side panel — compact on mobile (top), fixed-width on desktop (right) */}
      <aside className="order-1 sm:order-2 sm:w-48 flex-shrink-0 self-start w-full">
        <div
          className={[
            'border-2 border-primary bg-card rounded-[12px] p-3 shadow-[4px_4px_0px_0px_hsl(var(--shadow))]',
            'flex flex-row sm:flex-col items-center sm:items-stretch gap-3 sm:gap-2',
            'min-h-[64px] sm:min-h-[140px]',
            showClaimPanel ? '' : 'invisible',
          ].join(' ')}
          aria-hidden={!showClaimPanel}
        >
          <div className="flex-1 sm:flex-none sm:text-center">
            ...
            <div className="text-3xl sm:text-5xl font-black leading-none text-foreground mt-1">
              {state.declaredSuit}
            </div>
          </div>
          {state.phase === 'challenging' && state.lastPlay ? (
            <div className="flex-1 sm:pt-2 sm:border-t sm:border-border text-xs text-muted-foreground min-w-0">
              <div className="font-semibold text-foreground truncate">
                {playerNames[state.lastPlay.playerId] ?? state.lastPlay.playerId}
              </div>
              <div className="mt-0.5">
                {t('claimedPlayed')}{' '}
                <span className="font-bold text-foreground">{state.lastPlay.count}</span>{' '}
                {t('cardsUnit')}
              </div>
            </div>
          ) : (
            <div className="flex-1 sm:pt-2 sm:border-t sm:border-border text-xs text-muted-foreground" />
          )}
        </div>
      </aside>
    </div>
