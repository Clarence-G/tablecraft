...
test.describe('Social — friend request accept and unfriend flow', () => {
  test('Alice sends request → Bob accepts → both see each other → Alice unfriends', async ({
    browser,
  }) => {
    ...
    await expect(bobPanelR.locator(`text=${zh.lobbyPanel.friends.empty.title}`)).toBeVisible({
      timeout: 5000,
    });
  ...
  test('guest user sees sign-in CTA in friends tab, not friend search', async ({ page }) => {
    ...
    await expect(panel.locator(`text=${zh.lobbyPanel.friends.guestEmpty}`)).toBeVisible({
      timeout: 5000,
    });
    // "Sign In" button links to login
    await expect(panel.getByRole('button', { name: zh.auth.signIn })).toBeVisible();
    // Friend search input is NOT rendered for guests
    await expect(panel.getByPlaceholder(zh.lobbyPanel.friends.searchPlaceholder)).not.toBeVisible();
  });
